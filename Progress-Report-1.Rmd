...
# Arrange the dataset by hours_mission in ascending order and print the lowest 20 with selected columns
lowest_20_missions <- astronauts %>%
  arrange(hours_mission) %>%
  tail(100)

# Print the selected columns for the lowest 20 missions
print(lowest_20_missions[, c("name", "mission_title", "hours_mission")])
```

```{r}
top_100_hours <- astronauts %>%
...
# Create a box plot of in_orbit vs. hours_mission
...
# Log-transform the hours_mission variable
log_hrsum_astronauts <- astronauts %>%
  mutate(log_total_hrs_sum = log(hours_mission))  # Adding 1 to avoid log(0)

```
```{r}
# Complete pooled is the hierachical model name
complete_pooled_sum <- stan_glm(
  log_total_hrs_sum ~ 1 + in_orbit, data = log_hrsum_astronauts,
  family = gaussian,
  chains = 4, iter = 5000*2, seed = 84735, refresh = 0)
```
```{r}
pp_check(complete_pooled_sum)
```

```{r}
# Complete pooled is the hierachical model name
complete_pooled <- stan_glm(
  log_hours_mission ~ in_orbit_group , data = log_transformed_astronauts,
  family = gaussian,
  chains = 4, iter = 5000*2, seed = 84735, refresh = 0)
...
```
```{r}
prior_poisson <- stan_glm(
  log_hours_mission ~ in_orbit, data = log_transformed_astronauts,
  family = gaussian,
  prior_PD = TRUE,
  chains = 4, iter = 5000*2, seed = 84735, refresh = 0)
```
```{r}
pp_check(prior_poisson)
```
```{r}
library(dplyr)

# Filter out rows where in_orbit is "explosion"
filtered_astronauts <- astronauts %>%
  filter(in_orbit == "explosion")
...
model_1 <- stan_glmer(
  log_hours_mission  ~ in_orbit + (1 | name), 
...
```{r}
model_8 <- stan_glm(
  log_hours_mission ~ in_orbit + sex + year_of_mission + occupation + 1 , data = log_transformed_astronauts,
  family = gaussian,
  chains = 4, iter = 5000*2, seed = 84735, refresh = 0)
```
```{r}
...
<<<<<<< Updated upstream
We want to do it in a journal article format, but it is just the initial thought, might change as we progress. We are thinking about running few other models like poisson or normal, and try to fix the issue with this no good pp plot. 
=======
