...
log_transformed_astronauts %>% 
  filter(field21==0) %>% head()
```
field21 probably counts how many times astronauts exited their spacecraft to perform tasks outside during a particular mission. 
...
complete_pooled<- readRDS("models/new_hierachical.rds")
pp_check(hiera)
...
```{r}
# predict_cv <- prediction_summary_cv(model = complete_pooled, group = "in_orbit", data = log_transformed_astronauts, k = 5)

predict_cv
saveRDS(complete_pooled, "other_states/predict_cv.rds")
